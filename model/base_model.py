...
class BaseModel(object):
    ...
    def _get_where_num(self, output):
        ...
        # def sigmoid(x):
        #     return 1/(1 + np.exp(-x))
        relevant_prob = 1 - np.exp(output["column_funcw"][:, 0])
    ...
    def parse_output(self, input_feature, model_output, where_label = []):
        ...
        select_id_prob = sorted(enumerate(model_output["column_funcs"][:, 0]), key=lambda x:x[1], reverse=True)
        select = select_id_prob[0][0]
        agg = np.argmax(model_output["agg"][select, :])

        where_id_prob = sorted(enumerate(model_output["column_funcw"][:, 0]), key=lambda x:x[1], reverse=True)
